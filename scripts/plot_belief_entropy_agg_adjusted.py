...
Adjusted variant:
  - applies a manual upward shift to plotted entropy
  - enlarges standard-deviation bands for more visible shading
"""
...
AGENTS  = 35          # target the A20_* files
N_SCEN  = 5          # scen1..scen5
BASE    = "./results/belief_entropy"
ENTROPY_SHIFT = 0.6      # manual upward boost applied to mean entropy for all methods
STD_BAND_SCALE = 1.35    # >1.0 widens shaded uncertainty band
STD_SHADE_ALPHA = 0.22   # stronger shading than the original script

for DOMAIN in DOMAINS:
    OUT_PNG = f"./results/plots/{DOMAIN}_belief_entropy_A{AGENTS}_agg_adjusted.png"
    START_X_AT_ONE = True
    ...
    def main():
        ...
        for solver, label, color, lw in SOLVERS:
            ...
            upper = mu + sd
            plt.fill_between(x, lower, upper, color=color, alpha=STD_SHADE_ALPHA, zorder=1)
