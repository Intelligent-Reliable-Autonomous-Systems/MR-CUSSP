...
// int i = 0 ;
    while (open.empty() == false) {
        ...
        for(auto p_edge = outgoing_edges.begin(); p_edge != outgoing_edges.end(); p_edge++) {
            ...
            // Zero-cost wait self-loops with unchanged conflict count do not improve
            // the MO cost vector or conflict objective; expanding them creates unbounded
            // parent chains (stack overflow on teardown). Real wait actions that change
            // conflict_num (CBS CAT) are kept.
            if (p_edge->source == p_edge->target) {
                bool all_zero = true;
                for (size_t c : p_edge->cost) {
                    if (c != 0) {
                        all_zero = false;
                        break;
                    }
                }
                if (all_zero && next_ap->path_node->conflict_num == ap->path_node->conflict_num) {
                    continue;
                }
            }
