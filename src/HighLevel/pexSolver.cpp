...
//  main loop    
    ...
    if (is_success && !SOLUTIONS_cost.empty()) {
        const int sel = macussp::select_frontier_index(SOLUTIONS_cost, objective_ordering());
        if (sel >= 0 && sel < static_cast<int>(SOLUTIONS_waypoints.size())) {
            agent_paths = macussp::build_agent_paths_from_ids(SOLUTIONS_waypoints[static_cast<size_t>(sel)]);
            final_cost = SOLUTIONS_cost[static_cast<size_t>(sel)];
        }
    }
    return std::make_tuple(HLMergingTime, LowLevelTime, TotalTime, conflict_splits, SOLUTIONS_cost.size(), is_success, agent_paths, final_cost);
}