...
//  post process
    ...
    // Get paths from each agent start to goal
    AgentPaths agent_paths;
    for (int ag = 0; ag < AGENT_NUM; ++ag) {
        const auto& path = root_node->sop_waypoints.at(ag).at(root_node->cur_ids.at(ag));
        std::vector<std::tuple<int, int>> agent_path;
        for (const auto& node_id : path) {
            const auto& coord = id2coord[node_id];
            agent_path.emplace_back(coord[1], coord[0]);  // order flipped coz (row,col) ==> (y,x)
        }
        agent_paths[ag] = agent_path;
    }
    CostVector best_cost = root_node->cur_apex;
    
    return std::make_tuple(HLMergingTime, LowLevelTime, TotalTime, ConflictSolvingNum, hsolution_costs.size(), is_success, agent_paths, best_cost);
}