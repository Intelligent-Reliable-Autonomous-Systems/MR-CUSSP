...
void lcbsSolver::MergeJointPaths(HighLevelNodePtr node,
                                 int  solution_num,
                                 double /*max_eps not used any more*/)
{
    ...
    /* ---------- move into node->all_jps in lexicographic order --------- */
    apex_idx_combos.sort([](const auto& a,const auto& b){ return less_than_pair(a,b); });
...
void lcbsSolver::LexFilter(std::list<std::pair<CostVector,int>>& L,
                           ...
                           double eps6, double eps7, double eps8, double eps9, double eps10)
{
    ...
    const auto& ordering = objective_ordering();
    const int ord_len = static_cast<int>(ordering.size());

    for (int p = 0; p < ord_len; ++p) {
        const int j = ordering[static_cast<size_t>(p)];
        if (j < 0 || j >= d) continue;
        const double ej = (j < 10) ? eps[j] : 0.0;

        double m = std::numeric_limits<double>::infinity();
        for (const auto& pr : L) m = std::min(m, static_cast<double>(pr.first[static_cast<size_t>(j)]));

        L.remove_if([&](const auto& pr){
            return static_cast<double>(pr.first[static_cast<size_t>(j)]) > m * (1.0 + ej);
        });
...
void lcbsSolver::MergeUntil(std::list<std::pair<CostVector, int>>& apex_idx_combos, std::vector<CostVector>& real_costs_vector, int solution_num, double max_eps)
{
    ...
    //     std::cout << "[lcbsSolver.cpp - l185]Real Cost: ";
...
void lcbsSolver::MergeUntil(std::vector<CostVector>& apex_vectors, std::vector<CostVector>& real_costs_vector, int solution_num, double max_eps)
{
...
void lcbsSolver::calculate_CAT(HighLevelNodePtr node, VertexCAT& vertex_cat, EdgeCAT& edge_cat, int agent_id) // CAT - detaist about the constraints- Conflict Agent TIme
{
...
OutputTuple lcbsSolver::run(std::vector<Edge>& edges, std::vector<std::pair<size_t, size_t>>& start_goal, HSolutionID& hsolution_ids, std::vector<CostVector>& hsolution_costs, LoggerPtr& logger)
{
    ...
    const macussp::ObjectiveOrdering& lex_order = objective_ordering();
    ...
    while(!open_list.empty())
    {
        ...
        if(std::get<2>(cft).empty()){
            if(EAGER){
                ...
                // Update the best verified solution
                if (!have_best || macussp::lex_less(real_cost, best_cost, lex_order)) {
                    have_best = true;
                    best_cost = real_cost;
    ...
    printLCBSResult(is_success);
    if (is_success) {
        // print best cost
        for (int i = 0; i < best_cost.size(); ++i) {
            output << "Cost for o_" << (i + 1) << ": " << best_cost[i] << std::endl;
        }
    }
