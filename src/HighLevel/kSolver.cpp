...
void kSolver::calculate_CAT(HighLevelNodePtr node, VertexCAT& vertex_cat, EdgeCAT& edge_cat, int agent_id)
{
    ...
    for(int i = 0; i < node->cur_ids.size(); i++){
        ...
        auto path_it = node->sop_waypoints.at(i).find(node->cur_ids.at(i));
        if (path_it == node->sop_waypoints.at(i).end()) continue;
        const auto& path_nodes = path_it->second;
        for(int t = 0; t < static_cast<int>(path_nodes.size()); t++){
            size_t node_id = path_nodes.at(static_cast<size_t>(t));
            if (node_id >= GRAPH_SIZE) continue;
            if(!vertex_cat.count(t)){
                vertex_cat.insert(std::make_pair(t, std::vector<int>(GRAPH_SIZE, 0)));
            }
            vertex_cat.at(t).at(node_id) ++;
        }
        for(int t = 0; t < static_cast<int>(path_nodes.size()) - 1; t++){
            size_t source_id = path_nodes.at(static_cast<size_t>(t));
            size_t target_id = path_nodes.at(static_cast<size_t>(t + 1));
            if (source_id >= GRAPH_SIZE || target_id >= GRAPH_SIZE) continue;
            if(!edge_cat.count(t)){
...
//  initialize open_list
    ...
    if(difftime(time(NULL), start_time) > TIME_LIMIT){
        ...
        return std::make_tuple(HLMergingTime, LowLevelTime, difftime(time(NULL), start_time), 0, 0, false, empty_paths, CostVector{});
    }
...
//  main loop    
            ...
            for(auto iter = node->all_jps.begin(); iter != node->all_jps.end(); ){
                ...
                if(std::get<2>(cft).empty()){   // collision-free, use weakly dominate to prune
                    ...
                    if(!if_dominated){   
                        ...
                        for(int i = 0; i < AGENT_NUM; i ++){
                            const auto path_it =
                                node->sop_waypoints.at(i).find(iter->second.at(i));
                            if (path_it == node->sop_waypoints.at(i).end()) {
                                valid_paths = false;
                                break;
                            }
                            new_hsolution.push_back(path_it->second);
                        }
                        if (!valid_paths) {
    ...
    //  DomPrune
        ...
        if(std::get<2>(cft).empty()){
            if(EAGER){
                ...
                for(int i = 0; i < AGENT_NUM; i++){
                    const auto cost_it = node->sop_cost.at(i).find(node->cur_ids.at(i));
                    if (cost_it == node->sop_cost.at(i).end()) {
                        valid_costs = false;
                        break;
                    }
                    add_cost(real_cost, cost_it->second);
                }
                if (!valid_costs) {
                    continue;
                }
                ...
                std::vector<std::vector<size_t>> new_hsolution;
                for (int i = 0; i < AGENT_NUM; ++i) {
                    const auto path_it = node->sop_waypoints.at(i).find(node->cur_ids.at(i));
                    if (path_it == node->sop_waypoints.at(i).end()) {
                        valid_costs = false;
                        break;
                    }
    ...
    // Get paths from frontier solution selected by Theta(c_g)
    AgentPaths agent_paths;
    CostVector final_cost;
    if (is_success && !hsolution_costs.empty()) {
        const int sel = macussp::select_frontier_index(hsolution_costs, objective_ordering());
        if (sel >= 0 && sel < static_cast<int>(hsolution_ids.size())) {
            agent_paths = macussp::build_agent_paths_from_ids(hsolution_ids[static_cast<size_t>(sel)]);
            final_cost = hsolution_costs[static_cast<size_t>(sel)];
        }
    }
    
    return std::make_tuple(HLMergingTime, LowLevelTime, TotalTime, ConflictSolvingNum, hsolution_costs.size(), is_success, agent_paths, final_cost);
}