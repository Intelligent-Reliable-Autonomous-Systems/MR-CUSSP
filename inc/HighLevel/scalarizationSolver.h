...
class scalarizationSolver: public HighLevelSolver
{
protected:
    int SOLUTION_NUM;
    ...
    MergingStrategy MS;
    double EPS = 0;
    std::vector<double> scalar_weights_;

    double weighted_scalar_cost(const CostVector& c) const;


    void MergeJointPaths(HighLevelNodePtr node, int solution_num, double max_eps=INT_MAX);
    void scalarizationFilter(std::list<std::pair<CostVector,int>>& apex_idx_combos, double eps1, double eps2, double eps3, double eps4, double eps5, 
...
public:
    virtual ~scalarizationSolver() = default;
    scalarizationSolver(size_t graph_size, int agent_num, Algorithm algorithm, bool if_eager, int dim, int turn_dim, int turn_cost, int time_limit)
    ...
    void set_scalarization_weights(const std::vector<double>& weights) { scalar_weights_ = weights; }
    ...
    void printScalarizationResult(bool is_success) {
        const int box_width = 50;
        std::string box_color = is_success ? "\033[1;32m" : "\033[1;31m"; // Green for success, Red for failure
        std::string message = is_success
            ? "\033[1;32mSCALARIZATION Solver was successful 😄\033[0m"
            : "\033[1;31mSCALARIZATION Solver was unsuccessful 🙁\033[0m";

        int text_length = message.length() - 13;
        int padding = (box_width - 2 - text_length) / 2; // 2 for side borders
        ...
        std::cout << std::string(padding, ' ') << message
                << std::string(box_width - 2 - padding - text_length, ' ');
        std::cout << box_color << "│\033[0m\n";
