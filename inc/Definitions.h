...
enum Algorithm {BBMOCBS_EPS, BBMOCBS_PEX, BBMOCBS_K, LCBS, SCALARIZATION};
...
struct ApexPathPair {
    ...
    bool update_nodes_by_merge_if_bounded(const ApexPathPairPtr &other, const EPS eps, MergingStrategy s=MergingStrategy::SMALLER_G2);
...
public:
    ...
    std::vector<PathSet>                sop_waypoints;  // waypoints of each set of paths
    std::vector<CostSet>                sop_apex;
    std::vector<CostSet>                sop_cost;
    ...
    std::list<JointPathPair>            all_jps;
...
//  constructor
    HighLevelNode(size_t agent_num) : sop_waypoints(std::vector<PathSet>(agent_num)), 
            sop_apex(std::vector<CostSet>(agent_num)), sop_cost(std::vector<CostSet>(agent_num)),
            cur_ids(std::vector<size_t>(agent_num)), vertex_constraints(std::vector<VertexConstraint>(agent_num)),
...
using AgentPaths = std::unordered_map<int, std::vector<std::tuple<int,int>>>; // agent_id -> path (list of (x,y) tuples)
using OutputTuple = std::tuple<double, double, double, int, int, bool, AgentPaths, CostVector>;
